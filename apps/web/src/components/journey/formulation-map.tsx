...
export function FormulationMap({ formulation }: FormulationMapProps) {
  const { roots, recentActivators, perpetuatingCycles, protectiveStrengths } =
    formulation.formulation;

  const hasRoots = roots.length > 0 || recentActivators.length > 0;
  const hasCycles = perpetuatingCycles.length > 0;
  const hasStrengths = protectiveStrengths.length > 0;

  if (!hasRoots && !hasCycles && !hasStrengths) return null;

  return (
    <section className="rounded-2xl border border-foreground/10 bg-white p-6 shadow-sm">
      <h2 className="mb-1 text-lg font-semibold text-foreground">How We Understand This</h2>
      ...
      <div className="space-y-5">
            ...
            <div className="space-y-2">
              ...
              {recentActivators.map((act, i) => (
                ...
                >
                  ...
                  <span className="mt-1 inline-block text-[10px] text-amber-600/60">
                    recent trigger
                  </span>
                </div>
              ))}
        ...
        {hasStrengths && (
          <div>
            ...
            <div className="space-y-2">
              {protectiveStrengths.map((strength, i) => (
                <div
                  key={`str-${i}`}
                  className="rounded-lg border border-[#7c9a82]/15 bg-[#7c9a82]/5 px-3.5 py-2.5"
                >
                  <p className="text-sm leading-relaxed text-foreground/70">{strength.content}</p>
                  <span className="mt-1 inline-block text-[10px] text-[#7c9a82]/60">
                    {strength.sourceType.replace("_", " ")}
                  </span>
                </div>
              ))}
            </div>
