...
- **onEnd hook order**: `voice-post-session-analysis` (background) → `session-summary` (critical, user waits) → `formulation` (background) → `therapy-plan` (background) → `clinical-handoff-sync` (background) → `user-memory-blocks` (background) → `therapeutic-calibration` (background)
